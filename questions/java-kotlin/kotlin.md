...
Место, где нужен String, Int, любой другой тип — можно поставить Nothing, потому что он "
встраивается" в любой тип.
...
Если input — null, вместо возврата происходит throw, тип Nothing безопасно "встраивается" в любую
переменную.
...
Дженерики позволяют параметризовать типы, чтобы писать код, который работает с разными типами без
дублирования.
...
<h4>🧠 Стирание типов (type erasure)<h4/>
Что происходит под капотом:
На уровне JVM дженерики стираются.
После компиляции код с дженериками превращается так, как будто универсальные типы заменяются на
Object.
Компилятор вставляет приведения типов (cast) там, где это нужно.
...
- Нельзя сделать T::class
- Нельзя проверить item is T
- Нельзя создать новый экземпляр T() напрямую

[статья](https://javarush.com/quests/lectures/questsyntaxpro.level12.lecture06)

</details>

<details>
<summary>in, out в дженериках — ковариантность и контравариантность</summary>

in/out → управляют направлением использования типа (чтение/запись) для безопасности
ковариаций/контравариаций.
В Котлин, для параметризованных типов она встречается в трёх видах: инвариантность, ковариантность и
контрвариантность. Наследование для параметризованных типов возможно, но «по умолчанию» для наших,
только что написанных классов оно выключено.

<h4>out (ковариантность)<h4/>
...
<h4>in (контрвариантность)<h4/>
Контрвариантность представляют как обратный процесс ковариантности, это тоже способ включить
наследование, но работает он иначе.
...
Позволяет получить информацию о типе T в обобщённых функциях (обычно недоступно из-за type erasure
на JVM).

Под капотом - Компилятор подставляет реальный тип вместо T и создаёт нужные проверки.
...
<details>
<summary>Ключевое слово crossinline</summary>
Что делает:
Запрещает нелокальный return из лямбды.

Используется внутри inline-функций.

Почему важно:
Нелокальный return в таких случаях привёл бы к некорректному выходу из функции, которого уже нет в
стеке вызовов.

Когда ты не вызываешь лямбду напрямую, а:

- передаёшь её в другой поток (Thread)
- сохраняешь её в переменной
- вызываешь позже (delayed execution)

→ нелокальный return небезопасен и запрещён.
🔧 Чтобы это явно контролировать, Kotlin предлагает crossinline, который запрещает нелокальный return
внутри лямбды.
🚫 Если убрать crossinline, и в block() будет return, компилятор выдаст ошибку:
"A return is not allowed here"

🧠 Что под капотом
В обычных inline-функциях код вставляется на место вызова, поэтому return можно применить как goto
из тела лямбды в вызывающую функцию.
Если же лямбда сохраняется в поле или передаётся куда-то ещё, она не встраивается как часть исходной
функции, и return уже не сможет выйти из внешней функции.
crossinline защищает от этих ситуаций и заставляет лямбду быть безопасной.
